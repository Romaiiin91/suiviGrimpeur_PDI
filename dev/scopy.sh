...
rsync -avz --update "$LOCAL_SRC_DIR/" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/src/"

# Copier le dossier include
rsync -avz --update "$LOCAL_INCLUDE_DIR/" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/include/"

# Copier les fichiers spécifiques du dossier cgi-bin
rsync -avz --update "$LOCAL_CGI_BIN_DIR/action.c" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/server/cgi-bin/"
rsync -avz --update "$LOCAL_CGI_BIN_DIR/video.cpp" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/server/cgi-bin/"
rsync -avz --update "$LOCAL_CGI_BIN_DIR/utilsCgi.h" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/server/cgi-bin/"

# Copier le dossier html
rsync -avz --update "$LOCAL_HTML_DIR/" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/server/html/"

# Copier le fichier paramDetection.json
rsync -avz --update "$LOCAL_PARAM_FILE" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/data/"

# Copier le Makefile
rsync -avz --update "$LOCAL_MAKEFILE" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/"
